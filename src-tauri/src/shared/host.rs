...
pub struct Host {
    ...
    pub count: String,
    #[serde(skip)]
    host_node: Option<String>, // Store raw XML for lazy parsing
}
...
pub fn filter_hosts_by_status<'a>(hosts: &'a [Host], status: &str) -> Vec<&'a Host> {
    hosts.iter().filter(|h| h.status == status).collect()
}

pub fn filter_hosts_with_open_ports<'a>(
    hosts: &'a [Host],
    host_nodes: &'a [roxmltree::Node],
) -> Vec<(&'a Host, &'a roxmltree::Node<'a, 'a>)> {
    hosts
        .iter()
        .zip(host_nodes.iter())
