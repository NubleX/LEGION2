...
impl OS {
    ...
    pub fn from_nmap_xml(os_node: &roxmltree::Node) -> Self {
        let get_attr = |name: &str| -> String { os_node.attribute(name).unwrap_or("").to_string() };
        ...
        Self {
            ...
            accuracy: get_attr("accuracy").parse().unwrap_or(0),
        }
    }

    pub fn is_empty(&self) -> bool {
        self.name.is_empty() && self.family.is_empty() && self.vendor.is_empty()
    }
}
...
impl OSDetector {
    ...
    pub async fn detect_via_nmap(
        &self,
        target: &str,
    ) -> Result<Vec<OS>, Box<dyn std::error::Error>> {
    ...
    pub async fn detect_via_http_headers(
        &self,
        url: &str,
    ) -> Result<OS, Box<dyn std::error::Error>> {
...
impl PassiveOSDetector {
    pub fn analyze_tcp_signature(&self, tcp_signature: &TcpSignature) -> OS {
        ...
        if tcp_signature.tcp_options.contains(&"wscale".to_string()) {
