...
impl CorrelationEngine {
    ...
    pub async fn correlate_host_findings(&self, host_ip: &str) -> Result<Vec<Finding>> {
        ...
        for rule in &self.correlation_rules {
            if let Some(correlated) = self
                .apply_correlation_rule(rule, &existing_findings, host_ip)
                .await?
            {
        ...
        log::info!(
            "Generated {} correlated findings for host {}",
    ...
    pub async fn analyze_new_host(&self, host_ip: &str) -> Result<Vec<Finding>> {
        ...
        if self.is_likely_server(host_ip) {
            ...
            server_finding.description = format!(
                "Host {} appears to be a server based on IP pattern",
                host_ip
            );
    ...
    pub async fn correlate_all_findings(&self) -> Result<Vec<Finding>> {
        ...
        for (host_ip, host_findings) in findings_by_host {
            for rule in &self.correlation_rules {
                if let Some(correlated) = self
                    .apply_correlation_rule(rule, &host_findings, &host_ip)
                    .await?
                {
                    correlated_findings.push(correlated);
                }
        ...
        log::info!(
            "Generated {} correlated findings across network",
            correlated_findings.len()
        );
        Ok(correlated_findings)
    }
    ...
    fn load_default_correlation_rules() -> Vec<CorrelationRule> {
        vec![
            CorrelationRule {
                ...
                description: "Host has multiple open ports indicating potential attack surface"
                    .to_string(),
                triggers: vec![FindingTrigger {
            ...
            CorrelationRule {
                ...
                description: "Administrative services detected on network-accessible host"
                    .to_string(),
                triggers: vec![FindingTrigger {
                    finding_type: "service".to_string(),
                    host_pattern: None,
