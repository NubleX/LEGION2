...
use crate::utils::os::is_command_available;
use serde_json::json;
...
impl Source for MasscanScanner {
    ...
    async fn start(&self, plan: &Plan) -> Result<ObsStream> {
        ...
        let stream = stream::unfold(
            (lines, discovered_count, child, None),
            move |(mut lines, mut count, mut child, pending)| async move {
                if let Some(obs) = pending {
                    return Some((obs, (lines, count, child, None)));
                }
                ...
                match lines.next_line().await {
                    Ok(Some(line)) => {
                        ...
                        if let Some(obs) = parse_masscan_line(&line, scan_id) {
                            ...
                            let progress_obs = create_progress_observation(&line, scan_id, count);
                            Some((obs, (lines, count, child, Some(progress_obs))))
                        } else {
